...
def computeErrFunc(u1,u0,nSample,lag=80,norm='L2'):
    ...
    for ll in np.arange(-lag,lag + 1): # loop over the lags PS edited for 0 index
        thisLag = ll + lag #P.S. version -> python indexed from zero
        
        for ii in np.arange(nSample): # loop over the samples
    ...
    for ll in np.arange(-lag,lag+1): # loop over lags P.S. edited for 0 index
        thisLag = ll + lag #P.S. edited for 0 index
        
        for ii in np.arange(nSample): #loop over samples
...
def accumulateErrFunc(direction,err,nSample,lag=80,b=2):
    ...
    if direction > 0: 
        ...
        iInc   = 1
        loop   = np.arange(iBegin,iEnd + 1,iInc) # P.S. version
    else:
    ...
    for ii in loop:
        ...
        for ll in np.arange(nLag): # 0 -> 400
            #------------------------------------------------------------------
            ...
            if ji != jb: #equation 10 in Hale (2013)
                #for kb in np.arange(ji,jb + iInc,-iInc):
                for kb in np.arange(ji,jb + iInc + 1,-iInc): # P.S. version
...
def backtrackDistFunc(direction, d, err, lmin, b):
    ...
    stbar   = np.zeros((nSample)) # set space for warping function
    ...
    ll = np.argmin(d[iBegin,:]) # min accum dist at front/back based on 'dir'
    ...
    while ii != iEnd:
        ...
        if ji != jb: # equation 10 in Hale 2013
            ...
            for kb in np.arange(ji,jb-iInc + 1,iInc): # sum errors over i-1:i-b+1
                
                distLminus1 = distLminus1 + err[kb,lMinus1]
                distLplus1  = distLplus1  + err[kb,lPlus1]
        
        dl = np.min([distLminus1, distL, distLplus1]) # update min dist to previous sample
        ...
        if ll == lMinus1 or ll == lPlus1: # check edges to see about b values
            if ji != jb: # if b > 1 then need to move more steps
                for kb in np.arange(ji,(jb - iInc) + 1,iInc):
                    ii = ii + iInc
                    stbar[ii] = ll + lmin 
    return stbar
...
def computeDTWerr(Aerr, u, mLag):
    ...
    if np.shape(Aerr,1) != npts: #### check this #### 
