...
# edited March 2022 Parker Sprinkle dpsprink@uw.edu parker.sprinkle@pnnl.gov
def CreateGrid (seismic_data, grid_step):
    xls=[]
    ils=[]
    for i in range (0, seismic_data.shape[2], grid_step):
        xls.append(i)
    for i in range (0, seismic_data.shape[0], grid_step):
        ils.append(i)
    grid=[]
    for ii in xls:
        for jj in ils:
            seismictrace=seismic_data[jj,:,ii]
