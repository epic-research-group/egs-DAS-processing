...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan  3 14:40:17 2022
...
def data_subset(dat, centers, win):
    ...
    dat_sub =[np.arange(i) for i in win]
    for idx in range(num_traces):
        dat_sub[idx][:] = dat[int(centers[idx]-win[idx]):int(centers[idx]+win[idx]), idx]
...
def piterative_cc(dat, t_vec, init_picks, win, dt, acc_val, max_shift, max_iter):
